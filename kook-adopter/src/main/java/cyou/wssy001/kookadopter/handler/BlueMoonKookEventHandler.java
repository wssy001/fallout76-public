...
public class BlueMoonKookEventHandler implements BaseHandler {
    ...
    public BaseReplyMsgDTO consume(BaseEvent baseEvent, BasePlatformEventDTO basePlatformEventDTO) {
        if (basePlatformEventDTO instanceof KookEventDTO kookEventDTO) {
            ...
            if (CollUtil.isEmpty(blueMoonPicUrls)) {
                log.error("******BlueMoonKookEventHandler.consume：蓝月当空主题奖励清单图片获取失败");
                format = String.format(KookReplyMsgTemplateEnum.ERROR_MSG_CARD.getMsg(), "蓝月当空主题奖励清单图片获取失败，请联系管理员");
            } else {
                format = String.format(KookReplyMsgTemplateEnum.BLUE_MOON_CARD.getMsg(), blueMoonPicUrls.get("1"));
            }
