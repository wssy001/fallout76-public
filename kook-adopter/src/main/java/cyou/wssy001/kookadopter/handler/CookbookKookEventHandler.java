...
public class CookbookKookEventHandler implements BaseHandler {
    ...
    public BaseReplyMsgDTO consume(BaseEvent baseEvent, BasePlatformEventDTO basePlatformEventDTO) {
        if (basePlatformEventDTO instanceof KookEventDTO kookEventDTO) {
            ...
            if (CollUtil.isEmpty(cookbookPicUrls)) {
                log.error("******CookbookKookEventHandler.consume：常见食物图图片获取失败");
                format = String.format(KookReplyMsgTemplateEnum.ERROR_MSG_CARD.getMsg(), "常见食物图图片获取失败，请联系管理员");
            } else {
                format = String.format(KookReplyMsgTemplateEnum.COOKBOOK_CARD.getMsg(), cookbookPicUrls.get("1"), cookbookPicUrls.get("2"));
            }
